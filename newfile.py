...
# ============================================================
import os
TOKEN = os.getenv("BOT_TOKEN", "1234567890:YOUR_TOKEN_HERE")
...
def upgrade_building_transaction(user_id: int, building_column: str, cost: int, new_level: int) -> bool:
    with get_connection() as conn:
        cur = conn.cursor()
...
def show_battle_reports(chat_id: int, user_id: int, message_id: Optional[int] = None):
    with get_connection() as conn:
        ...
        # Optimize by grabbing opponent names upfront
        opp_ids = []
        for b in battles:
...
def background_jobs():
    ...
    while True:
        try:
            ...
            # Only spawn castles occasionally, not every 5 seconds!
            with get_connection() as conn:
                cur = conn.cursor()
